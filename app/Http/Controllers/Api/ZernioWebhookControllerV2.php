...
class ZernioWebhookControllerV2 extends Controller
{
 private const AI_SYSTEM_PROMPT = <<<'PROMPT'
Tu es l'assistant virtuel de ClassiNote, une application mobile qui aide les parents À suivre la scolarité de leurs enfants au Togo.

QUI TU ES :
- Tu es un assistant amical, serviable et professionnel
- Tu parles en français, de façon simple et concise (2-4 lignes maximum)
- Tu utilises des emojis avec modération pour rendre la conversation agréable
- Tu utilises UNIQUEMENT *un astérisque* pour le gras (*texte*), JAMAIS deux

CE QUE TU SAIS :
- ClassiNote coÀte 1000 FCFA/mois par enfant
- Fonctionnalités : notes, absences, emploi du temps, frais scolaires, annonces, messagerie avec les profs, remarques des enseignants
- Les parents peuvent consulter les notes en temps réel
- Suivi des absences et retards
- Gestion des paiements scolaires
- Communication directe avec les enseignants

COMMENT TU RÉPONDS :
- Salutations â†' salue poliment et propose ton aide
- Questions sur les fonctionnalités â†' explique et guide vers l'action ("envoyez *notes*" par exemple)
- Questions sur le tarif â†' 1000 FCFA/mois
- Questions générales â†' réponds du mieux que tu peux avec tes connaissances
- Confusion â†' aide l'utilisateur À trouver ce qu'il cherche
- Remerciements â†' de rien, avec plaisir !

IMPORTANT : Tu dois TOUJOURS répondre. Ne dis jamais "je ne peux pas" ou "je ne sais pas" sans proposer une alternative. Si tu ne connais pas la réponse, redirige poliment vers l'administration de l'école.
PROMPT;

 private const AI_PARENT_PROMPT = <<<'PROMPT'
Tu es l'assistant virtuel de ClassiNote pour un parent inscrit. L'enfant sélectionné s'appelle {enfant_prenom}, mais le parent peut avoir plusieurs enfants.

QUI TU ES :
- Tu es un assistant amical, serviable et professionnel
- Tu parles en français, de façon simple et concise (1-2 lignes maximum)
- Tu utilises des emojis avec modération
- Tu utilises UNIQUEMENT *un astérisque* pour le gras (*texte*), JAMAIS deux

CE QUE TU SAIS :
- ClassiNote coÀte 1000 FCFA/mois
- Fonctionnalités : notes, absences, emploi du temps, frais, annonces, messagerie profs, remarques, paiements

COMMENT TU RÉPONDS :
- Salutations â†' salue poliment en 1 ligne (NE PAS utiliser le prénom de l'enfant)
- Questions â†' réponds en 1-2 lignes maximum
- Remerciements â†' de rien, avec plaisir !

IMPORTANT :
- NE JAMAIS lister toutes les options
- NE JAMAIS commencer par "Bonjour [prénom]"
- Sois BREF et CONCIS (1-2 lignes max)
PROMPT;
 ...
 $this->sendText($conv, "✅ *Image bien reçue !*\n\n Classe : {$classeName}\n Matiere : {$matiereName}\n\nNous traiterons vos notes sous peu. Merci !");
 $conv->resetState();
 ...
 // â"€â"€â"€ Normal parent flow â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
 $parent = $this->findParentByPhone($phoneCandidates);
 $prof = $this->findProfByPhone($phoneCandidates);

 // â"€â"€â"€ Rate limiting (10 messages/minute) â"€â"€â"€â"€â"€â"€â"€â"€â"€
 ...
 // â"€â"€â"€ Prof + Parent dual role â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
 $role = $conv->state_data['role'] ?? null;

 if ($state === 'awaiting_role_selection' || $state === 'awaiting_prof_action' || $state === 'awaiting_prof_image' || $state === 'awaiting_prof_eval_type' || $state === 'awaiting_prof_eval_select') {
 if ($prof) {
 ...
 ['title' => 'âœ… Oui, j\'essaie', 'payload' => 'demo_confirm'],
 ['title' => 'âŒ Non, merci', 'payload' => 'demo_cancel'],
 ]);
 return;
 }
 ...
 $this->sendText($conv, "Ž‰ *Votre compte est activé !*\n\nRavi de vous accueillir sur ClassiNote.");
 ...
 $this->sendText($conv, "Bien sÀr ! Pour quel enfant souhaitez-vous consulter {$what} ?", $buttons);
 }
 ...
 . "0 Ce lien sécurisé expire dans 10 minutes.";
 ...
 // â"€â"€â"€ MENU RICHE â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
 ...
 $afterButtons[] = ['title' => ''â€+ Mode Prof', 'payload' => 'role_prof'];
 }
 ...
 $lines[] = "â€ Le *{$date}*{$heure}{$remarque}";
 }
 ...
 // Ecolage
 $lines[] = "Ž" *Écolage (scolarité) :*";
 $lines[] = "â€ Montant : *" . number_format($ecolage, 0, ',', ' ') . " FCFA*";
 $lines[] = "â€ Payé : " . number_format($scolaritePaye, 0, ',', ' ') . " FCFA — reste : " . ($resteEcolage <= 0 ? 'âœ…' : number_format($resteEcolage, 0, ',', ' ') . " FCFA");
 ...
 $statut = $reste <= 0 ? 'âœ…' : number_format($reste, 0, ',', ' ') . " FCFA";
 $lines[] = "â€ {$f->libelle} : *" . number_format($f->montant, 0, ',', ' ') . " FCFA* (payé : " . number_format($paye, 0, ',', ' ') . " FCFA — reste : {$statut})";
 }
 ...
 if ($nbAbsences > 0) {
 $lines[] = "0 *{$nbAbsences}* absence(s) récente(s)";
 }
 ...
 $lines = ["'â€+ *Enseignants de {$eleve->prenom}* :\n"];
 $rawProfs = [];
 ...
 $lines[] = "â€ *" . number_format($p->montant, 0, ',', ' ') . " FCFA* — {$label}";
 ...
 $lines[] = "â€ *{$exam->titre}* ({$type})";
 $lines[] = " "š {$matiere} — "… {$date}";
 }
 ...
 $message .= "0 Suivre les absences et retards\n";
 ...
 $message .= "â€ Notes de 3 matières\n";
 $message .= "â€ Emploi du temps\n";
 $message .= "â€ Absences fictives\n";
 $message .= "â€ Frais scolaires\n\n";
 $message .= "0 *Important :* Ce compte et toutes les données seront automatiquement supprimés dans *1 heure*.\n\n";
 ...
 $message = "Ž‰ *Merci pour votre retour positif !*\n\n";
 ...
 $this->sendText($conv, "âŒ Une erreur est survenue lors de la création du compte démo.", [
 ...
 if (!$school) {
 $this->sendText($conv, "âŒ Erreur lors de la création du compte démo.");
 return;
 }

 $annee = $school->anneesScolaires()->where('active', true)->first();
 if (!$annee) {
 $this->sendText($conv, "âŒ Aucune année scolaire active.");
 return;
 }

 $classes = $school->classes()->where('annee_scolaire_id', $annee->id)->take(2)->get();
 if ($classes->isEmpty()) {
 $this->sendText($conv, "âŒ Aucune classe disponible.");
 return;
 }
 ...
 'contenu' => 'Une réunion parents-professeurs est prévue le 25 aoÀt À 15h dans la salle de conférence. Votre présence est souhaitée.',
 'type' => 'info',
 'publie' => true,
 ]);
 ...
 $conv->resetState();
 $this->sendText($conv, "âœ… *Compte démo créé !*\n\n' *Amina Dupont* — {$classe1->libelle}\n' *Kofi Dupont* — {$classe2->libelle}\n\n Notes, absences, examens À venir, remarques, frais et paiements générés.\n\n0 Ce compte expire dans *1 heure*.\n\nEnvoyez *menu* pour explorer !");
 ...
 $ligne = "â€ {$matiere} : *{$note->note}/{$noteSur}*";
 ...
 if (empty($eleveIds)) {
 return "0 Aucune absence enregistrée pour le moment.\n\nRépondez Â menu Â pour revenir au menu.";
 }
 ...
 return "âœ… Aucune absence ni retard enregistré. Bravo, votre enfant est assidu !\n\nRépondez Â menu Â pour revenir au menu.";
 }

 $lines = ["0 *Absences / retards* :"];
 $currentEleve = null;
 ...
 return "'â€+ Aucun enseignant trouvé pour le moment.\n\nRépondez Â menu Â pour revenir au menu.";
 }
 ...
 if ($profs->isEmpty()) {
 return "'â€+ Aucun enseignant trouvé pour la classe de votre enfant.\n\nRépondez Â menu Â pour revenir au menu.";
 }

 $lines = ["'â€+ *Les enseignants de votre enfant* :"];
 ...
 $lines[] = "Répondez Â menu Â pour revenir au menu.";
 return implode("\n", $lines);
 }
 ...
 // Ecolage
 $lines[] = "Ž" Écolage : *" . number_format($ecolage, 0, ',', ' ') . " FCFA*";
 $lines[] = " Payé : " . number_format($scolaritePaye, 0, ',', ' ') . " FCFA — reste : " . ($resteEcolage <= 0 ? 'âœ…' : number_format($resteEcolage, 0, ',', ' ') . " FCFA");
 ...
 $lines[] = "â€ Aucun frais enregistré pour le moment.";
 }
 }
 ...
 $ligne = "\nâ€ {$payment['date']} — *" . number_format($payment['montant'], 0, ',', ' ') . " FCFA*";
 ...
 $message .= "'â€+ Professeur — " . ($schoolName?->nom ?? 'École') . "\n";
 ...
 $this->sendText($conv, $message, [
 ['title' => ''â€+ Mode Prof', 'payload' => 'role_prof'],
 ...
 // â"€â"€â"€ Class selection from menu â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
 ...
 ['title' => '0 Marquer absences', 'payload' => 'prof_absences'],
 ...
 // â"€â"€â"€ Image received (photo for notes) â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
 if ($state === 'awaiting_prof_image' || $state === 'awaiting_prof_eval_type' || $state === 'awaiting_prof_eval_select') {
 if ($norm === 'role_parent' || $norm === 'role parent' || preg_match('/mode.*parent|parent.*mode/i', $norm)) {
 $conv->setState('awaiting_menu_action', ['role' => 'parent']);
 $parent = $this->findParentByPhone([$conv->participant_phone]);
 if ($parent) $this->startRichMenu($conv, $parent);
 ...
 // â"€â"€â"€ Menu actions â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
 if ($norm === 'role_prof') {
 $this->sendProfMenu($conv, $prof);
 return;
 }
 ...
 $schoolName = $schools->first()?->nom ?? 'École';
 $affectations = $prof->affectations()->with('classe')->get();
 ...
 $this->sendText($conv, "'â€+ *M. {$prof->prenom} {$prof->nom}* — {$schoolName}", [], [
 'type' => 'list',
 'body' => ['text' => "'â€+ *M. {$prof->prenom} {$prof->nom}* — {$schoolName}\n\nSélectionnez une classe ou une action :"],
 'action' => [
 'name' => 'single_select',
 'button' => 'Voir les options',
 'sections' => $sections,
 ...
 'expires_at' => now()->addMinutes(10),
        ...
        $url = config('zernio.public_url') . "/magic/prof/dashboard?t={$token}";
 ...
 'expires_at' => now()->addMinutes(10),
        ...
        $url = config('zernio.public_url') . "/magic/prof/emploi?t={$token}";
 ...
 $url = config('zernio.public_url') . "/magic/prof/absences?t={$token}";
 ...
 ['title' => 'â" Interrogation', 'payload' => 'prof_interro'],
 ['title' => '"™ Retour', 'payload' => 'menu'],
 ]);
 return;
 }
 ...
 private function handleProfEvalType($conv, $prof, string $norm): void
 {
 if ($norm === 'retour' || $norm === 'menu') {
 $this->sendProfMenu($conv, $prof);
 return;
 ...
 private function handleProfEvalSelect($conv, $prof, string $norm): void
 {
 if ($norm === 'retour' || $norm === 'menu') {
 $this->sendProfMenu($conv, $prof);
 return;
 ...
 if (preg_match('/eval[:\s]+(\d+)/i', $norm, $m)) {
 $evalId = (int) $m[1];
 ...
 $classe = \App\Models\Classe::find($eval->classe_id);
 $this->sendText($conv, 'Envoyez une photo de votre tableau de notes pour ' . ($classe?->libelle ?? 'votre classe') . '. Indiquez la matiere et la colonne si necessaire.', [
 ['title' => 'Retour', 'payload' => 'menu'],
 ]);
 return;
 }
