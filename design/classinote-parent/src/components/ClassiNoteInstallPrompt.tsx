...
export const ClassiNoteInstallPrompt: React.FC = () => {
  ...
  useEffect(() => {
    const dismissedAt = localStorage.getItem('classinote_parent_install_dismissed');
    if (dismissedAt) {
  ...
  const handleDismiss = () => {
    setShowPrompt(false);
    setIsDismissed(true);
    localStorage.setItem('classinote_parent_install_dismissed', Date.now().toString());
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-[#00113a]">Installer ClassiNote</h3>
            <p className="text-xs text-[#757682]">Accès rapide à l'espace parents</p>
          </div>
        </div>
