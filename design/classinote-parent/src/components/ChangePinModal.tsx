    ...
    <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-6 shadow-2xl max-w-sm w-full space-y-5">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-[#00113a]">Changer le PIN</h3>
          <button
            onClick={handleClose}
        ...
        {success ? (
          <div className="text-center py-4 space-y-4">
            ...
            <div>
              <p className="text-sm font-bold text-[#00113a]">PIN modifié avec succès</p>
              <p className="text-xs text-slate-500 mt-1">Votre nouveau PIN est maintenant actif.</p>
            </div>
            <button
              onClick={handleClose}
              className="w-full h-12 bg-[#002366] hover:bg-[#00113a] text-white font-bold text-xs rounded-2xl transition-colors cursor-pointer"
            >
              Fermer
            </button>
          </div>
          ...
          <div className="space-y-4">
            ...
            <input
              ...
              placeholder="****"
              className="w-full px-4 py-4 text-center text-3xl font-mono font-bold tracking-[0.5em] bg-slate-50 border border-slate-200 rounded-2xl text-[#00113a] placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-[#002366] transition-all"
            />

            {error && (
              <p className="text-xs text-rose-600 text-center font-medium bg-rose-50 p-2 rounded-xl border border-rose-200">{error}</p>
            )}
            ...
            {step === 'confirm' ? (
              <button
                ...
                className="w-full h-12 bg-[#002366] hover:bg-[#00113a] text-white font-bold text-xs rounded-2xl flex items-center justify-center gap-2 shadow-lg disabled:opacity-40 cursor-pointer transition-all"
              >
                {loading ? (
