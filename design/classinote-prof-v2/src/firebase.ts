...
export async function initFirebaseMessaging() {
  ...
  } catch { return { messaging: null, registration: null }; }
}
...
export async function requestPushPermission(registration?: ServiceWorkerRegistration | null) {
  try {
    ...
    if (token) {
      await apiFetch('/teacher/device-token', { method: 'POST', body: JSON.stringify({ device_token: token }) });
      return true;
    }
    return false;
  } catch { return false; }
...
export async function removePushToken(): Promise<boolean> {
  try { if (messaging) await deleteToken(messaging); await apiFetch('/teacher/device-token', { method: 'DELETE' }); return true; }
  catch { return false; }
}
